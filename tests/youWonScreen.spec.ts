...
it.describe("Win menu page testing", () => {
  it.beforeEach(async ({ page }) => {
    await page.goto("http://localhost:8000/");
    await page.evaluate(() => {
        window.next_state("player_won"); // force player won state
    });
  });
  ...
  it.describe("RESTART BTN testing", () => {
    ...
    it("button text", async ({ page }) => {
      await expect(page.locator("#but_a")).toHaveText("Restart");
    });
